...
class GameWindow < Gosu::Window
	...
	def search(cell_x ,cell_y)
		...
		if (cell_x == ((MAP_WIDTH / CELL_DIM) - 1))
			...
			if (cell_x > 0 and @columns[cell_x - 1][cell_y].vacant() and !@columns[cell_x - 1][cell_y].visited())
				@columns[cell_x - 1][cell_y].visited = true;
				west_path = search(cell_x - 1, cell_y);
			end
			if (cell_x < @columns.length() - 1 and @columns[cell_x + 1][cell_y].vacant() and !@columns[cell_x + 1][cell_y].visited())
				@columns[cell_x + 1][cell_y].visited = true;
				east_path = search(cell_x + 1, cell_y);
			end
			if (cell_y > 0 and @columns[cell_x][cell_y - 1].vacant() and !@columns[cell_x][cell_y - 1].visited())
				@columns[cell_x][cell_y - 1].visited = true;
				north_path = search(cell_x, cell_y - 1);
			end
			if (cell_y < @columns[0].length() - 1 and @columns[cell_x][cell_y + 1].vacant() and !@columns[cell_x][cell_y + 1].visited())
				@columns[cell_x][cell_y + 1].visited = true;
